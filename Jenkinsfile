        ...
        stage('Deploy') {
            steps {
                container(name: 'kubectl', shell: '/bin/bash') {
                    ...
                    sh "sed -i 's|NAme|${DOCKER_IMAGE_NAME}|' k8s/deployment.yaml"
                    sh "sed -i 's|BUild|${BUILD_NUMBER}|' k8s/deployment.yaml"
...
spec:
  ...
  - name: ubuntu
    ...
    - '-c'
    - |
      apt-get update && apt-get install -y curl #встановнення curl при створенні контейнера
"""
                }
            }
            steps {
